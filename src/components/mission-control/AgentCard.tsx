      ...
      <div className="flex items-start gap-3">
        ...
        >
          <Icon
            className={cn(
              "h-3.5 w-3.5",
              agent.status === AgentStatus.ACTIVE ? "text-signal" : "text-muted-foreground",
            )}
          />
        </div>
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <span
              className={cn(
                ...
                agent.status === AgentStatus.ACTIVE && "text-signal drop-shadow-[0_0_6px_var(--signal)]",
              )}
            >
              {agent.name}
            </span>
          ...
          <div className="mt-1.5 font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
            Last contact · {formatUtcShort(agent.lastContact)} · {timeAgo(agent.lastContact)}
          </div>

          {agent.currentMission && (
            <p className="mt-2 flex items-start gap-1.5 text-xs text-foreground/80">
              {isBlocked && <AlertTriangle className="mt-0.5 h-3 w-3 shrink-0 text-warning" />}
